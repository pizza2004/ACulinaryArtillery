    ...
    public class ItemSuperFood : Item
    {
        public override void OnHeldInteractStop(float secondsUsed, ItemSlot slot, EntityAgent byEntity, BlockSelection blockSel, EntitySelection entitySel)
        {
            if (byEntity.World is IServerWorldAccessor && GetNutritionProperties(byEntity.World, slot.Itemstack, byEntity) != null &&
                Attributes?["additionalNutritionProperties"]?.AsObject<FoodNutritionProperties[]>() is FoodNutritionProperties[] addProps && addProps?.Length > 0 && secondsUsed >= 0.95f)
            {
        ...
        public override void GetHeldItemInfo(ItemSlot inSlot, StringBuilder dsc, IWorldAccessor world, bool withDebugInfo)
        {
            ...
            if (GetNutritionProperties(world, stack, entity) != null && Attributes?["additionalNutritionProperties"]?.AsObject<FoodNutritionProperties[]>() is FoodNutritionProperties[] addProps && addProps?.Length > 0)
            {
                ...
                foreach (FoodNutritionProperties props in addProps)
                {
                    ...
                    if (Math.Abs(props.Health * healthLossMul) <= 0.001f) dsc.AppendLine(Lang.Get("efrecipes:- {0} {1} sat", Math.Round(props.Satiety * satLossMul), props.FoodCategory.ToString()));
                    else dsc.AppendLine(Lang.Get("efrecipes:- {0} {2} sat, {1} hp", Math.Round(props.Satiety * satLossMul), props.Health * healthLossMul, props.FoodCategory.ToString()));
                }
            }
        }
    }
